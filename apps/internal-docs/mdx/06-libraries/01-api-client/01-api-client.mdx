...
The `@odnlabs/api-client` package is essentially a class that provides methods to easily interact with the backend api. This package is used by the `website` and `web-client` applications.

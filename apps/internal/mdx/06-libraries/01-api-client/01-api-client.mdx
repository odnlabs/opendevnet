...
The `@odnlabs/api-client` package is essentially a class that provides methods to easily interact with the backend api. This package is used by the `site` and `web` applications.

## Usage

```ts
import { Client } from '@odnlabs/api-client';
import { ChannelType } from '@odnlabs/utils';
